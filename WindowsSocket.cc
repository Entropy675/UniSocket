...
{
	// Load the Windows socket library dynamically
	hWinSockLibrary = LoadLibrary("ws2_32.dll");
	if (!hWinSockLibrary) {
		std::cout << "WindowsSocket::WindowsSocket(): Failed to load the Windows socket library." << std::endl;
		throw "WindowsSocket::WindowsSocket(): Failed to load the Windows socket library.";
	}
	
	// Load function pointers
	_WSAStartup = (int (*)(WORD, LPWSADATA))GetProcAddress(hWinSockLibrary, "WSAStartup");
	_WSACleanup = (void (*)())GetProcAddress(hWinSockLibrary, "WSACleanup");
	_socket = (SOCKET (*)(int, int, int))GetProcAddress(hWinSockLibrary, "socket");
	_closesocket = (int (*)(SOCKET))GetProcAddress(hWinSockLibrary, "closesocket");
	_bind = (int (*)(SOCKET, const struct sockaddr*, int))GetProcAddress(hWinSockLibrary, "bind");
	_accept = (SOCKET (*)(SOCKET, struct sockaddr*, int*))GetProcAddress(hWinSockLibrary, "accept");
	_listen = (int (*)(SOCKET, int))GetProcAddress(hWinSockLibrary, "listen");
	_send = (int (*)(SOCKET, const char*, int, int))GetProcAddress(hWinSockLibrary, "send");
	_recv = (int (*)(SOCKET, char*, int, int))GetProcAddress(hWinSockLibrary, "recv");

	if (!_WSAStartup || !_WSACleanup || !_socket || !_closesocket || !_bind ||
		!_accept || !_listen || !_send || !_recv) {
		std::cout << "WindowsSocket::WindowsSocket(): Failed to obtain function pointer(s) from the Windows socket library." << std::endl;
		throw "WindowsSocket::WindowsSocket(): Failed to obtain function pointer(s) from the Windows socket library.";
	}
...
WindowsSocket::~WindowsSocket()
{
	...
	if (hWinSockLibrary) 
	{
        ...
        hWinSockLibrary = nullptr;
    }
	
	{
		std::lock_guard<std::mutex> lock(clientSocketsMutex);
		for(auto& sockets : clientSockets)
			closesocket(static_cast<SOCKET>(sockets));
	}
	
	// Cleanup and close socket
	_closesocket(static_cast<SOCKET>(serverSocket));
	
}

int WindowsSocket::sendData(const std::string& data)
{
	return _sendData(data, -1); // Send to all clients using other func and invalid socket
}
...
{
	// Create a socket
	SOCKET tmp = _socket(AF_INET, SOCK_STREAM, 0);
	...
	// Bind the socket
	if (_bind(tmp, (struct sockaddr*)&serverAddr, sizeof(serverAddr)) == SOCKET_ERROR) 
	{
